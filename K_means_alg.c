...
// 'num' is given number, mean[] is set of k-means &
// fn call gives index of cluster to which num will be assign.
int Assign_cluster(float num,float mean[],int k)
{
    float dist,min; int i,temp;
    for(i=1; i<=k; i++)
        { dist=(float)pow((num-mean[i]),2);
...
float Mean(node *ll) // Fn. returns the mean of all eles. of linked list
{
    node *t=ll; float mean=0; int i=0;
    while(t!=NULL)
        { mean=mean+t->data; t=t->next; i++; }
    mean=(float)(mean/i);
    return mean;
}
...
{
    int i=1,index,j=1,k,l,m=0,n=0,p,q,t=0,stop=1; float sse,p_mean[10],c_mean[10],max[20],min[20],a[800][20],b[800]; char c,*s,*s1,temp[10]; FILE *f; node *ll[10];
    ...
    printf("Number of clusters: "); scanf("%d",&k); // For 'k' clusters reads means.
    printf("Mean of clusters:\n");
    for(q=1; q<=k; q++)
        { printf("Mean-%d) ",q); scanf("%f",&c_mean[q]); p_mean[q]=0; ll[q]=NULL; }

    for(q=1; q<l; q++) // assign all data to corr. cluster.
        {for(p=1; p<i; p++)
            { index=Assign_cluster(a[p][q],c_mean,k); 
            ll[index]=addnode(ll[index],a[p][q]); } }

    for(q=1; q<=k; q++) // Find of new means of all clusters.
        { p_mean[q]=c_mean[q];
        c_mean[q]=Mean(ll[q]); 
        if(p_mean[q]!=c_mean[q]) { stop=0; } }
    ...
    for(q=1; q<=k; q++)
        { printf("Cluster:%d) with mean=%f\n",q,c_mean[q]); print(ll[q]); print_file(ll[q],f); }
